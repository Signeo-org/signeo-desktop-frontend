...
ipcMain.handle("open-subtitle-window", async () => {
  ...
  if (process.env.NODE_ENV === "development") {
    await subtitleWindow.loadURL("http://localhost:5173/subtitle");
  } else {
    await subtitleWindow.loadFile(
...
ipcMain.handle("open-sign-window", async () => {
  ...
  if (process.env.NODE_ENV === "development") {
    await win.loadURL("http://localhost:5173/sign");
  } else {
    await win.loadFile(
      path.join(__dirname, "../renderer/index.html"),
      { hash: "/sign" }
...
ipcMain.handle("start-audio-tool", async () => {
...
ipcMain.handle("stop-audio-tool", async () => {
  ...
  if (!response.ok) {
    throw new Error("Failed to stop audio tool on server");
  }
  return true;
});
